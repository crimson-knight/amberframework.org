...
test -s app/assets/brand/amber-crystal.svg
test -s app/assets/characters/amber-hero-desk-transparent-higgsfield.webp
test -s app/assets/characters/amber-frontier-higgsfield.webp
test -s app/assets/characters/amber-id-original-studio.webp
test -s app/assets/characters/amber-chibi-hero-mark-v2.webp
test -s app/assets/css/amber-brand.css
test -s app/assets/js/amber-site.js
...
grep -F 'Request-aware work stealing is available since beta.3 and remains disabled by default.' src/controllers/home_controller.cr
grep -F 'Test V2 beta' src/views/layouts/_nav.ecr
grep -F "menuButton.setAttribute('aria-label', open ? 'Close navigation' : 'Open navigation');" app/assets/js/amber-site.js
...
grep -F '.table-scroll' app/assets/css/amber-brand.css
grep -F 'padding: 1.15rem 1.25rem;' app/assets/css/amber-brand.css
...
grep -F 'white-space: nowrap;' app/assets/css/amber-brand.css
...
for sprite in productivity performance happiness humility respect trust; do
  ...
  test -s "app/assets/characters/way-sprites/${sprite}-b.webp"
done
...
if rg -n 'code\s*\{[^}]*padding:.*!important|pre code\s*\{[^}]*padding:\s*0' app/assets/css/amber-brand.css; then
